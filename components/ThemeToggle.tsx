 ...
 *
 * Renders in two places that never coexist: the mobile top bar below md and
 * the desktop sidebar at md and up. The breakpoint alone tells them apart, so
 * the label goes icon-only below md (sr-only keeps the accessible name) and
 * the padding only pads where it needs to be a tap target.
 */
export default function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggle}
      className="theme-toggle flex items-center gap-2 text-text-muted hover:text-accent transition-colors text-xs p-1.5 md:p-0"
    >
      <span data-when="dark" className="flex items-center gap-2">
        ...
        <span className="sr-only md:not-sr-only">Light theme</span>
      </span>
      <span data-when="light" className="flex items-center gap-2">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} aria-hidden="true" className="w-4 h-4 shrink-0">
          <path strokeLinecap="round" strokeLinejoin="round" d="M21.752 15.002A9.718 9.718 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z" />
        </svg>
        <span className="sr-only md:not-sr-only">Dark theme</span>
      </span>
    </button>
  )
}
